...
crudControllers.controller('ApplicationDetailCtrl', function($rootScope,$scope,$location,$routeParams,ajaxFactory,$controller)
{
	...
	{
		$location.path("/"+$routeParams.apiname+"/"+$scope.item.id+"/set/1");
	};*/

	$scope.editAppComp = function(item) 
	{
		$location.path("/"+$routeParams.apiname+"/"+$scope.item.id+"/set/2");
	};

	$scope.editCommunications = function(item) 
	{
		$location.path("/"+$routeParams.apiname+"/"+$scope.item.id+"/set/3");
	};
